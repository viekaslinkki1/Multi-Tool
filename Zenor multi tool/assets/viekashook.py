...
black = "\033[1;30m"
titletext = " [-- VIEKASHOOK --] Made by github.com/viekas_linkki1"
red = "\033[1;31m"
green = "\033[1;32m"
yellow = "\033[1;33m"
blue = "\033[1;34m"
...
cyan = "\033[1;36m"
white = "\033[1;37m"
invalidurl = f"{red}[! VIEKASHOOK !]{white} Invalid url!"
# test = "" test webhook, dont forget to remove :3

socials = {
    "github": {"link": "https://github.com/viekaslinkki1"},
}  # You can update this list, and it will dynamically update.

logo = """
 __          __  _     _                 _      _____                              __      _____  
 \ \        / / | |   | |               | |    |  __ \                             \ \    / /__ \ 
  \ \  /\  / /__| |__ | |__   ___   ___ | | __ | |  | | ___ _ __ ___   ___  _ __    \ \  / /   ) |
   \ \/  \/ / _ \ '_ \| '_ \ / _ \ / _ \| |/ / | |  | |/ _ \ '_ ` _ \ / _ \| '_ \    \ \/ /   / / 
    \  /\  /  __/ |_) | | | | (_) | (_) |   <  | |__| |  __/ | | | | | (_) | | | |    \  /   / /_ 
     \/  \/ \___|_.__/|_| |_|\___/ \___/|_|\_\ |_____/ \___|_| |_| |_|\___/|_| |_|     \/   |____|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
    >> [Webhook Multitool developed by @viekas_linkki1]
"""
...
# def sendembed(url):
#     tit = input(f"{yellow}[? VIEKASHOOK ?]{white}Title for the embed: ")
#     des = input(f"{yellow}[? VIEKASHOOK ?]{white}Description: ")
#     color = input(f"{yellow}[? VIEKASHOOK ?]{white}Hex-Color: ")
...
def changepfp(url):
    input(f"{yellow}[? VIEKASHOO ?]{white} Press enter to select file or skip this to input the path/url")
    image_path = fd.askopenfilename(filetypes=[("Profile Pictures", "*.png;*.jpg;*.jpeg")])
    if image_path is None or image_path == "":
        clear()
        image_path = input(f"{yellow}[? VIEKASHOOK ?]{white} Path/URL to image: ")
    ...
    try:
        ...
        response.raise_for_status()
        print(f"{green}[+ VIEKASHOOK +]{white} Profile picture changed successfully.")
    except FileNotFoundError:
        print(f"{red}[! VIEKASHOOK !] File not found. Please provide a valid file path or image url.")
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! VIEKASHOOK !] HTTP Error: {errh}")
    except requests.exceptions.ConnectionError as errc:
        print(f"{red}[! VIEKASHOOK !] Error Connecting: {errc}")
    except requests.exceptions.Timeout as errt:
        print(f"{red}[! VIEKASHOOK !] Timeout Error: {errt}")
    except requests.exceptions.RequestException as err:
        print(f"{red}[! VIEKASHOOK !] Request Exception: {err}")

def deletehook(url):
    print(f"{cyan}[+ VIEKASHOOK +]{white} Trying to delete webhook...")
    try:
        response = requests.delete(url)
        response.raise_for_status()  # Raises HTTPError for bad responses
        print(f"{green}[+ VIEKASHOOK +]{white} Webhook deleted successfully.")
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! VIEKASHOOK !] HTTP Error: {errh}")
...
def sendmessage(url):
    msg = input(f"{yellow}[? VIEKASHOOK ?]{white} Message: ")
    try:
        response = requests.post(url, json={"content": msg})
        response.raise_for_status()
        print(f"{green}[+ VIEKASHOOK +]{white} Message sent successfully.")
...
def renamehook(url):
    name = input(f"{yellow}[? VIEKASHOOK ?]{white} Webhook Name: ")
    print(f"{cyan}[+ VIEKASHOOK +]{white} Trying to change username...")
    try:
        response = requests.patch(url, json={"name": name})
        response.raise_for_status()
        print(f"{green}[+ VIEKASHOOK +]{white} Webhook name changed successfully.")
...
def spamhook(url):
    print(f"{cyan}[+ VIEKASHOOK +]{white} Trying to spam webhook...")
    msg = input(f"{yellow}[? VIEKASHOOK ?]{white} Spam Text: ")
    timeout = float(input(f"{yellow}[? VIEKASHOOK ?]{white} Timeout (to avoid api-ratelimit): "))
    try:
        print(f"{red}[! VIEKASHOOK !] Spam has started, Relaunch the tool to stop spam and use it again.")
        while True:
            response = requests.post(url, json={"content": msg})
            response.raise_for_status()
            print(f"{green}[+ VIEKASHOOK +]{white} Sent message")
            time.sleep(timeout)
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! VIEKASHOOK !] HTTP Error: {errh}")
...
with open(f"{os.getcwd()}\\src\\viekas.txt", "w+") as file:
...
webhook = {}
os.system("title github.com/viekaslinkki1")
while True:
    clear()
    printascii()
    while True:
    ...
    while True:
        ...
        print(f"\n\n\n{green}[+ VIEKASHOOK +]{white} Logged into webhook: {webhook_name}")
        ...
        if ch == 1:
            ...
            print(f"{cyan}[+ VIEKASHOOK +]{white} Source code can be found here:")
            for platform, info in socials.items():
                link = info["link"].replace("https://", "")
